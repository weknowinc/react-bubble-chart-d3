...
export default class BubbleChart extends Component {
  ...
  renderBubbles(width, nodes, color) {
    ...
    // Add image at center of bubble
    const imagePercentage = 0.6; // Adjust this percentage as needed (60% = 0.6)

    node
      .append("image") // Appending image element for each node
      .attr("xlink:href", function (d) {
        // Set the URL/path of the image you want to render
        return d.data.imageUrl; // Replace 'imageUrl' with the property holding the image URL in your data
      })
      .attr("x", function (d) {
        // Position the image at the bottom center horizontally
        return (-d.r * imagePercentage) / 2; // Adjust as needed for horizontal positioning
      })
      .attr("y", function (d) {
        // Position the image at the bottom center vertically
        return d.r * 0.001; // Adjust as needed for vertical positioning
      })
      .attr("width", function (d) {
        // Set the width of the image based on a percentage of the bubble's radius
        return d.r * imagePercentage; // Adjust as needed for the image size
      })
      .attr("height", function (d) {
        // Set the height of the image based on a percentage of the bubble's radius
        return d.r * imagePercentage; // Adjust as needed for the image size
      })
      .style("display", function (d) {
        const self = d3.select(this);
        const width = self.node().getBBox().width;
        d.hideLabel = width < 20;
        return d.hideLabel ? "none" : "block";
      });

    // Position the texts inside the circles.
    d3.selectAll(".label-text")
      ...
      .style("opacity", function (d) {
        const self = d3.select(this);
        const width = self.node().getBBox().width;
        d.hideLabel = width > d.r * 2;
        return d.hideLabel ? 0 : 1;
      })
      .attr("y", function (d) {
        return -labelFont.size * 0.2;
      });
